...
export function Footer() {
  return <footer className="foundation-footer"><div className="shell"><div className="foundation-footer-top"><Brand /><nav aria-label="Footer navigation">{links.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</nav></div><div className="foundation-footer-bottom"><p>© {new Date().getFullYear()} The SozoRock Foundation, Inc. A U.S. 501(c)(3) public charity.</p><nav aria-label="Legal and policy links"><Link href="/privacy">Privacy Notice</Link><Link href="/terms">Website Terms</Link><Link href="/accessibility">Accessibility</Link><Link href="/nondiscrimination">Nondiscrimination</Link><Link href="/standards">Standards</Link><Link href="/support">Support our work</Link></nav></div></div></footer>;
}
