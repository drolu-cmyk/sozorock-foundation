...
Production uses the documented OpenAI-compatible APIs on Amazon Bedrock in a bounded approved US region. Deployment first probes `openai.gpt-5.6-sol` through the Responses-only Bedrock Mantle endpoint, then falls back to `us.anthropic.claude-sonnet-4-6` through Bedrock Runtime or `openai.gpt-oss-120b` and `openai.gpt-oss-20b` through Bedrock Mantle Chat Completions. Runtime and Mantle apply separate per-model quota pools. The runtime selects the matching Agents SDK transport before the six-agent canary. IAM is restricted to Mantle inference on the account-default projects and to the corresponding Claude inference-profile/foundation-model and GPT-OSS foundation-model ARNs, plus short-term bearer-token use. The runtime retains the same agent graph, bounded revisions, and evaluator gates. The exact production Lambda derives a short-term Bedrock API key from its IAM role using `@aws/bedrock-token-generator`. The key is generated for a graph run, is never persisted or logged, and expires with the underlying AWS session.

The deployer probes GPT-5.6 Sol in `us-east-2` and `us-east-1`, with Claude Sonnet 4.6 and GPT-OSS fallbacks across `us-east-2`, `us-east-1`, and `us-west-2`, and pins the first healthy regional endpoint. This bounded US-region failover avoids coupling production activation to one model's regional quota while preserving the approved models and IAM constraints.
