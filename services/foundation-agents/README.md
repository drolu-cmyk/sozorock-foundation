...
Production uses Amazon Bedrock's OpenAI-compatible Responses API in `us-east-1`. Deployment runs a tiny real Responses probe and selects the lowest-latency account-available model from the bounded approved set (`openai.gpt-oss-20b`, with `openai.gpt-oss-120b` as fallback) before the six-agent canary. GPT-OSS runs on the Bedrock runtime `/openai/v1/responses` endpoint because that endpoint supports structured outputs; Mantle does not expose that feature for these models. Stable model names are mapped to Bedrock's versioned runtime identifiers (`-1:0`) for inference. The runtime retains the same agent graph, bounded revisions, and evaluator gates. The exact production Lambda derives a short-term Bedrock API key from its IAM role using `@aws/bedrock-token-generator`. The key is generated for a graph run, is never persisted or logged, and expires with the underlying AWS session. The Lambda role is restricted to the two approved model ARNs and `SHORT_TERM` bearer-token use.
