...
import { routes, indexableRoutes } from '../site/content.mjs';
...
await cp(new URL('../site/leadership-reference.css', import.meta.url), new URL('./leadership-reference.css', out));
...
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${indexableRoutes.map(route => `  <url><loc>${origin}${route === '/' ? '/' : route}</loc></url>`).join('\n')}\n</urlset>\n`;
...
await writeFile(new URL('./site.webmanifest', out), JSON.stringify({
  name:'The SozoRock Foundation',
  short_name:'SozoRock Foundation',
  start_url:'/',
  display:'standalone',
