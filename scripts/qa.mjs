...
const deferredVisualPaths = new Set([
  ...
  '/assets/rrg-cover.jpg',
  '/assets/rebs-cover.jpg'
]);
  ...
  for (const [, src] of html.matchAll(imgPattern)) {
    if (src.startsWith('/')) {
      try { await access(resolve(root, `.${src}`)); }
        ...
        if (deferredVisualPaths.has(src)) warnings.push(`${route}: deferred approved visual not yet bundled ${src}`);
        else failures.push(`${route}: missing local image ${src}`);
      }
    }
  }
...
for (const needle of [
  ...
  '<span class="brand-mark"><span class="brand-word"><span class="brand-name">sozorock</span><span class="brand-reg"'
]) {
...
for (const image of [
  ...
  catch { warnings.push(`/leadership: approved portrait deferred ${image}`); }
}
...
for (const image of ['/assets/hsa-cover.webp','/assets/rrg-cover.jpg','/assets/rebs-cover.jpg']) {
  ...
  try { await access(resolve(root, `.${image}`)); }
    ...
    if (deferredVisualPaths.has(image)) warnings.push(`/publications: publication cover deferred ${image}`);
    else failures.push(`/publications: publication cover missing ${image}`);
  }
}
...
if (!rrg.includes('10.65473/rrg-v1-2025') || !rrg.includes('9798993647715') || !rrg.includes('/assets/rrg-cover.jpg')) failures.push('/publication/rrg-v1-2025: incomplete permanent record or real cover wiring');
const rebs = await htmlFor('/publication/rebs-v1-2025');
if (!rebs.includes('10.65473/rebs-v1-2025') || !rebs.includes('9798993647708') || !rebs.includes('/assets/rebs-cover.jpg')) failures.push('/publication/rebs-v1-2025: incomplete permanent record or real cover wiring');
...
console.log(`PASS ${routes.length} routes; release contracts satisfied with ${warnings.length} deferred visual warning(s).`);
